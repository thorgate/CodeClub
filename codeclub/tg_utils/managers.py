...
class ClosableObjectsQuerySet(models.QuerySet):
    ...
    """
    def close(self, user):
        return super().get_queryset().update(closed_at=timezone.now(), closed_by=user)


class NotClosedObjectsManager(models.BaseManager.from_queryset(ClosableObjectsQuerySet)):
...
NotifyModificationsManager.__doc__ = NotifyPostChangeQuerySet.__doc__
